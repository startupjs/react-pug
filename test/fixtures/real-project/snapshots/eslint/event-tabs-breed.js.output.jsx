...
const SelectLikes = observer(({ $likes, oppositeBreed, eventId }) => {
  ...
  return ($cats.map(($cat) => ((() => {const catId = $cat.getId();return <Item styleName={["item", { selected: $likes[catId].get() }]} key={catId} onPress={() => $likes[catId].get() ? $likes[catId].del() : $likes[catId].set(true)}><CatCard $cat={$cat} small={true} /></Item>;})())))
  styl`
    .item
      border-radius 1u
      &.selected
