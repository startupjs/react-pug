...
export default observer(({ breed }) => {
  ...
  return (
    <>
      ...
      <Content padding={1}>
        <Button onPress={showCreate}>Add new {breed}</Button>
      </Content>
      <Modal
            ...
            <>
              ...
              {$mode.get() === 'new'
                ...
                : $mode.get() === 'edit'
                  ? (
                    <Div align='right' row>
                      <Button color='error' onPress={deleteCat}>
                        Delete
                      </Button>
                    </Div>
                    )
          ...
          <Item.Right>
            <Div vAlign='center' row gap={1}>
              ...
              <Link href={'/cats/' + $cat.token.get()}>
                <Button variant='text' icon={faLink} tooltip='Cat profile link'>
                  Link
                </Button>
              </Link>
            </Div>
...
const SelectLikesInput = observer(({ $value, ...props }) => {
  ...
  return oppositeBreed
    ...
    : (
      <Alert variant='warning'>Select breed to choose likes</Alert>
      )
})
...
const SelectLikes = observer(({ $likes, oppositeBreed, eventId }) => {
  ...
  return (() => {
    ...
    return __pugEachResult.length
      ...
      : (
        <Alert variant='info'>No cats with selected breed yet</Alert>
        )
  })()
})
