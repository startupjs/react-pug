...
export default observer(({
  ...
  return <Div part={'root'} row={true} vAlign={'center'}>{photoFileId ? <Photo styleName={["avatar", {
      large,
      small
    }]} fileId={photoFileId} name={name} /> : <Avatar styleName={["avatar", {
      large,
      small
    }]}>{name}</Avatar>}<Div row={true}><Span styleName={["text", {
        large
      }]} bold={true}>{(number || 'X') + '. '}</Span><Div><Span styleName={["text", {
          large
        }]}>{name}</Span>{showPhone ? <>{phone ? <Span styleName={["text", {
            large
          }]}><Span bold={true}>Phone:{' '}</Span>{phone}</Span> : null}{catgram ? <Span styleName={["text", {
            large
          }]}><Span bold={true}>Catgram:{' '}</Span><Link styleName={["text", {
              large
            }]} to={getCatgramLink(catgram)}>{catgram}</Link></Span> : null}{phonegram ? <Span styleName={["text", {
            large
          }]}><Span bold={true}>Phonegram:{' '}</Span><Link styleName={["text", {
              large
